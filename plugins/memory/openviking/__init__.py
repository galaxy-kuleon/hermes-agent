...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._account = ""
        self._user = ""
        # A gateway may host many end users while sharing one process-level
        # OPENVIKING_USER.  The request identity is authoritative for memory
        # isolation and must survive config hot reloads.
        self._user_override = ""
        self._agent = ""
        self._session_id = ""
        self._turn_count = 0
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        self._user_override = str(kwargs.get("user_id") or "").strip()
        self._user = self._user_override or settings["user"]
    ...
    def _ensure_client_locked(self) -> Optional["_VikingClient"]:
        ...
        # Never let process-level config refresh collapse a multi-user gateway
        # back into the shared configured namespace.
        user = self._user_override or settings["user"]
