...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        try:
            ...
            # on one code path.
            pass
        # chown may clear setgid bits, and some hosts refuse to set setgid
        # while the directory still has an inherited foreign group. Apply the
        # final contract mode only after ownership has converged.
        path.chmod(mode)
...
class S6ServiceManager:
    ...
    def _render_run_script(
        ...
        The script:
          ...
          2. Restores ``HOME`` from ``HERMES_CONTAINER_HOME`` before the
             privilege drop. This stays distinct from profile-scoped
             ``HERMES_HOME`` and from the agent working directory.
