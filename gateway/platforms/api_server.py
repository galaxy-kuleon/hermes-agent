    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        try:
            ...
            if "user_id" not in cols:
                ...
                logger.info(
                    "[api_server] Migrated ResponseStore.conversations to include user_id column"
                )
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        if idempotency_key:
            ...
            scoped_idem_key = (
                f"{user_id}:{idempotency_key}" if user_id else idempotency_key
            )
            try:
                result, usage = await _idem_cache.get_or_set(
                    scoped_idem_key, fp, _compute_completion
                )
            except Exception as e:
                logger.error(
            ...
            async def _emit(item):
                ...
                * Plain strings — standard ``delta.content`` chunks.
                ...
                if (
                    ...
                    isinstance(item, tuple)
                    and len(item) == 2
                    and item[0] == "__reasoning_delta__"
                ):
                    text = item[1]
                    chunk = {
                        "id": completion_id,
            ...
            *,
            ...
            if conversation:
                self._response_store.set_conversation(
                    conversation, response_id, user_id=user_id
                )
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        if idempotency_key:
            ...
            try:
                result, usage = await _idem_cache.get_or_set(
                    scoped_idem_key, fp, _compute_response
                )
            except Exception as e:
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        session_id = _scope_session_id(
            body_session_id or stored_session_id or run_id, scope
        )
        self._touch_session_activity(session_id, scope)
    ...
    def _load_session_activity(self) -> None:
        ...
        logger.info(
            "[api_server] reloaded %d session activity records (of %d on disk) from %s",
            len(kept),
            len(loaded),
            path,
        )

    def _maybe_commit_sibling_sessions(
        self, session_id: str, user_id: str, chat_id: str
    ) -> None:
        ...
        for sid, info in snapshot:
            ...
            if loop is not None:
                loop.create_task(
                    self._commit_session_async(sid, info.get("user_id", ""))
                )
                logger.info(
    ...
    def _commit_openviking_session_sync(self, session_id: str, user_id: str) -> bool:
        ...
        try:
            ...
            logger.debug(
                "[api_server] httpx unavailable; cannot commit OpenViking session"
            )
            return False
