...
def test_viking_client_sends_tenant_headers_first_for_trusted_mode(monkeypatch):
    client = _VikingClient(
        "https://example.com",
        api_key="test-key",
        account="acct",
...
def test_queue_prefetch_sends_limit_not_legacy_top_k():
    ...
    assert captured_payloads == [
        {"query": "anything", "limit": 5, "target_uri": "viking://user/usr/"},
        {"query": "anything", "limit": 5, "target_uri": "viking://resources/"},
    ]
    assert all("top_k" not in payload for payload in captured_payloads)
