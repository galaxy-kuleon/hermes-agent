...
def test_gateway_user_override_survives_connection_hot_reload(monkeypatch, tmp_path):
    """A shared gateway config must never collapse distinct users' memories."""
    _clear_openviking_env(monkeypatch)
    monkeypatch.setenv("OPENVIKING_ENDPOINT", "http://first.test:1933")
    monkeypatch.setenv("OPENVIKING_USER", "shared-process-user")
    constructed = []

    class FakeVikingClient:
        def __init__(self, endpoint, api_key="", account="", user="", agent=""):
            constructed.append((endpoint, user))

    monkeypatch.setattr(openviking_module, "_VikingClient", FakeVikingClient)
    monkeypatch.setattr(
        openviking_module,
        "_classify_runtime_openviking_health",
        lambda client, endpoint: ("healthy", "ok"),
    )
    ...
    assert provider._user == "owui-user-a"
    assert constructed[-1] == ("http://first.test:1933", "owui-user-a")

    monkeypatch.setenv("OPENVIKING_ENDPOINT", "http://second.test:1933")
    assert provider._ensure_client() is not None
    assert provider._user == "owui-user-a"
    assert constructed[-1] == ("http://second.test:1933", "owui-user-a")
